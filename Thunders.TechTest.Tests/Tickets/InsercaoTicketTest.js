...
function getRandomTipoVeiculo() {
    return Math.floor(Math.random() * 3); // Random integer between 0 and 2
}

export default function () {
    let url = 'https://localhost:7405/api/Ticket/'; // URL do endpoint
    let randomPedagioId = pedagioIds[Math.floor(Math.random() * pedagioIds.length)];
    let payload = JSON.stringify({
        pedagioId: randomPedagioId,
        tipoVeiculo: getRandomTipoVeiculo(), // Substitua pelo valor apropriado
        dataUtilizacao: getRandomDateIn2025().toISOString() // Data aleatória no ano de 2025
    });
